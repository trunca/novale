...
eDBoxLCD::eDBoxLCD()
{
   ...
   if (fd >= 0) {
       ...
       close(fd);        
       if(!strncmp(tmp,"gb800se\n",rd)||!strncmp(tmp,"gb800seplus\n",rd)||!strncmp(tmp,"gbultrase\n",rd))
           strcpy(sdevice,"/dev/mcu");
   }else{
       eDebug("[eDboxLCD] cannot open /proc/stb/info. Use fallback via demux count!");   		
   }
   
        eDebug("[eDboxLCD] Device: %s", sdevice);

	lcdfd = open(sdevice, O_RDWR);
	if (lcdfd < 0)
	{
