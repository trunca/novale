...
gettext.bindtextdomain('enigma2', '/usr/share/enigma2/po/')
...
def getConfig():
    ...
    if uh is False:
        print(_('Could not retrieve speedtest.net configuration'))
        sys.exit(1)
    configxml = []
    while 1:
    ...
    try:
        ...
        print(_('Failed to parse speedtest.net configuration'))
        sys.exit(1)
    del root
    del configxml
    return config
...
def closestServers(client, all=False):
    ...
    if not servers:
        print(_('Failed to retrieve list of speedtest.net servers'))
        sys.exit(1)
...
def speedtest():
    ...
    if not args.simple:
        print(_('Retrieving speedtest.net configuration...'))
    try:
        config = getConfig()
    except URLError:
        print(_('Cannot retrieve speedtest configuration'))
        sys.exit(1)

    if not args.simple:
        print(_('Retrieving speedtest.net server list...'))
    ...
    if args.server:
        ...
        try:
            ...
            print(_('Invalid Speedtest Mini URL'))
            sys.exit(1)
        else:
            text = f.read()
            f.close()
        ...
        if not urlparts or not extension:
            print(_('Please provide the full URL of your Speedtest Mini server'))
            sys.exit(1)
        servers = [{
    ...
    if not args.simple:
        ...
        try:
            unicode()
            print _('Hosted by:\n'),(('%(sponsor)s' % best).encode('utf-8', 'ignore')),_('in'),(('%(name)s [%(d)0.2f km] -> '
                   '%(latency)s ms' % best).encode('utf-8', 'ignore'))
        except NameError:
            print _('Hosted by:\n'),(('%(sponsor)s' % best),_('in'),('(%(name)s) [%(d)0.2f km]: '
                   '%(latency)s ms' % best))
    else:
    ...
    if not args.simple:
        print_(_('\nTesting download speed'), '', end="")
    dlspeed = downloadSpeed(urls, args.simple)
    if not args.simple:
        print_()
    print _('Download:'),_('%0.2f M%s/s' %
    ...
    for size in sizesizes:
        for i in range(0, 25):
            sizes.append(size)
    if not args.simple:
        print_(_('\nTesting upload speed'), '', end="")
    ulspeed = uploadSpeed(best['url'], sizes, args.simple)
    if not args.simple:
        print_()
    print _('Upload:'),_('%0.2f M%s/s' %
           ((ulspeed / 1000 / 1000) * args.units[1], args.units[0]))

    if args.share and args.mini:
        print(_('Cannot generate a speedtest.net share results image while '
        ...
        if f is False:
            print(_('Could not submit results to speedtest.net'))
            sys.exit(1)
        response = f.read()
        code = f.code
        f.close()
        ...
        print(_('Share results: https://www.speedtest.net/result/%s.png' %
               resultid[0]))
