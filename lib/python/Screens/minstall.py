...
from Components.config import config, ConfigText, ConfigYesNo, ConfigSelection
...
config.userdir = ConfigText(default="/ipk/", visible_width = 70, fixed_size = False)
config.filtername = ConfigYesNo(default = False)
config.multifilemode = ConfigSelection(default = "Multi", choices = [
		("Multi", _("Multi files")),
...
<screen name="IPKToolsScreen2" position="center,160" size="750,370" title="IPK Tools">
	<ePixmap position="20,358" zPosition="1" size="170,2" pixmap="/usr/share/enigma2/skin_default/iconos/red.png" transparent="1" alphatest="on" />
	<ePixmap position="190,358" zPosition="1" size="200,2" pixmap="/usr/share/enigma2/skin_default/iconos/green.png" transparent="1" alphatest="on" />
	...
	def mList(self):
		self.list = []
		onepng = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/tar.png")
		treepng = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipk.png")
		sixpng = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipk3.png")
		fivepng = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipk4.png")
	...
	def clear(self):
		...
		if fileExists("/proc/mounts"):
			for line in open("/proc/mounts"):
				if "/dev/sd" in line:
					if pathExists(line.split()[1].replace('\\040', ' ') + config.userdir.value): 
						pathmp.append(line.split()[1].replace('\\040', ' ') + config.userdir.value)
		pathmp.append("/tmp/")
...
<widget source="menu" render="Listbox" position="15,10" size="820,455" scrollbarMode="showOnDemand">
	...
	<ePixmap position="20,512" zPosition="1" size="170,2" pixmap="/usr/share/enigma2/skin_default/iconos/red.png" transparent="1" alphatest="on" />
	<ePixmap position="190,512" zPosition="1" size="170,2" pixmap="/usr/share/enigma2/skin_default/iconos/green.png" transparent="1" alphatest="on" />
	...
	def feedlist(self):
		...
		statuspath = ''
		pkg_name = pkg_desc = ' '
		png = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipkmini.png")
...
<widget source="menu" render="Listbox" position="15,10" size="820,455" scrollbarMode="showOnDemand">
	...
	<ePixmap name="red" position="20,512" zPosition="1" size="170,2" pixmap="/usr/share/enigma2/skin_default/iconos/red.png" transparent="1" alphatest="on" />
	<ePixmap name="green" position="190,512" zPosition="1" size="220,2" pixmap="/usr/share/enigma2/skin_default/iconos/green.png" transparent="1" alphatest="on" />
	<ePixmap name="yellow" position="410,512" zPosition="1" size="220,2" pixmap="/usr/share/enigma2/skin_default/iconos/yellow.png" transparent="1" alphatest="on" />
...
<screen name="InstallAll4" position="center,160" size="750,405" title="Press -Info- to update plugin list">
  ...
  <ePixmap position="20,393" zPosition="1" size="165,2" pixmap="/usr/share/enigma2/skin_default/iconos/red.png" transparent="1" alphatest="on" />
  <ePixmap position="185,393" zPosition="1" size="165,2" pixmap="/usr/share/enigma2/skin_default/iconos/green.png" transparent="1" alphatest="on" />
  <ePixmap position="350,393" zPosition="1" size="200,2" pixmap="/usr/share/enigma2/skin_default/iconos/yellow.png" transparent="1" alphatest="on" />
  <ePixmap position="550,393" zPosition="1" size="190,2" pixmap="/usr/share/enigma2/skin_default/iconos/blue.png" transparent="1" alphatest="on" />
  ...
  <widget source="key_blue" render="Label" position="550,363" zPosition="2" size="190,30" valign="center" halign="center" font="Regular;22" transparent="1" />
	...
	def __init__(self, session, args=None):
		Screen.__init__(self, session)
		if config.multifilemode.value is 'Multi':
		...
		self.status = False 
		self.ipkminipng = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipkmini.png")
		self.tarminipng = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/tarmini.png")
		self["menu"] = List(self.list)
		self.nList()
		self["actions"] = ActionMap(["OkCancelActions", "ColorActions"],
			{
		...
		self["key_blue"] = StaticText(_("Descomprimir"))
		...
		if fileExists("/proc/mounts"):
			for line in open("/proc/mounts"):
				if "/dev/sd" in line:
					...
					searchPaths.append(line.split()[1].replace('\\040', ' ') + config.userdir.value)
		...
		if line_old is not None:
			if not line_old[-2]:
				if ".ipk" in line_old[0]:
					pngfile = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipkact.png")
					self.commamd_line_ipk.append(line_old[-1])
				else:
					pngfile = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/taract.png")
					self.commamd_line_tar.append('tar -C/ -xzpvf %s' % line_old[-1])
				self.status = True
			else:
				if ".ipk" in line_old[0]:
					pngfile = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipkmini.png")
					self.commamd_line_ipk.remove(line_old[-1])
				else:
					pngfile = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/tarmini.png")
	...
	def all_install(self):
		line_old = self["menu"].getCurrent()
		if line_old is not None:
			if config.multifilemode.value is not 'Multi':
				self.commamd_line_tar = []
				self.commamd_line_ipk = []
	...
	def desipk(self):
		os.system("rm -r /tmp/des")
		os.system("mkdir /tmp/des")
		os.system("ar x %s" % self["menu"].getCurrent()[-1])
		os.system("tar xvzf /home/root/data.tar.gz -C /tmp/des/")
		os.system("tar xvzf /home/root/control.tar.gz -C /tmp/des/")
		os.system("rm -r /home/root/*.tar.gz")
		os.system("rm -r /home/root/debian-*")
		self.mbox = self.session.open(MessageBox, _("%s descomprimido" % self["menu"].getCurrent()[-1]), MessageBox.TYPE_INFO, timeout = 4 )
...
<widget source="menu" position="15,10" render="Listbox" size="720,500">
	...
	<ePixmap position="20,558" zPosition="1" size="170,2" pixmap="/usr/share/enigma2/skin_default/iconos/red.png" transparent="1" alphatest="on" />
	<ePixmap position="190,558" zPosition="1" size="170,2" pixmap="/usr/share/enigma2/skin_default/iconos/green.png" transparent="1" alphatest="on" />
	<ePixmap position="360,558" zPosition="1" size="200,2" pixmap="/usr/share/enigma2/skin_default/iconos/yellow.png" transparent="1" alphatest="on" />
	...
	def nList(self):
		self.list = []
		ipkminipng = LoadPixmap(cached=True, path="/usr/share/enigma2/sfpanel/ipkmini.png")
