...
from SFtoolbox import getImageDistro
...
class TunerServer(Screen):
    ...
    def __init__(self, session):
        ...
        Screen.setTitle(self, _('Tunerserver Setup'))
    ...
    def ServStart(self):
        if os.path.ismount('/media/usb'):
            ...
            self.session.open(MessageBoxSF, _("Sorry, but you need to have a device mounted at '/media/usb'"), MessageBoxSF.TYPE_INFO)
    ...
    def doServStart(self):
        ...
        mytext = "Server avaliable on ip %s\nTo access this box's tuners you can connect via Lan or UPnP.\n\n1) To connect via lan you have to mount the /media/usb folder of this box in the client /media/usb folder. Then you can access the tuners server channel list from the client Media player -> Harddisk -> tuner.\n2) To connect via UPnP you need an UPnP server that can manage .m3u files like Mediatomb." % self.ip
        self['lab1'].setText(_(mytext))
        self.session.open(MessageBoxSF, _('Build complete!'), MessageBoxSF.TYPE_INFO)
        self.updateServ()

    def poPulate(self, bouquet, count):
        n = '%03d_' % count
        name = n + self.cleanName(bouquet[1])
        path = '/media/usb/tuner/' + name
        os.mkdir(path, 493)
        serviceHandler = eServiceCenter.getInstance()
    ...
    def ServStop(self):
        if self.my_serv_active == True:
            ...
            if os.path.exists('/media/usb/tuner'):
                rmtree('/media/usb/tuner')
            mybox = self.session.open(MessageBoxSF, _('Tunerserver disabled.'), MessageBoxSF.TYPE_INFO)
            mybox.setTitle(_('Info'))
            self.updateServ()
        self.session.open(MessageBoxSF, _('Server now disabled!'), MessageBoxSF.TYPE_INFO)
