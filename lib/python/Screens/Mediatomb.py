...
class MediatombPanel(Screen):
    skin = '\n\t<screen position="center,center" size="560,450" title="Mediatomb Panel">\n\t\t<widget source="list" render="Listbox" position="0,10" size="560,450" scrollbarMode="showOnDemand">\n\t\t\t<convert type="TemplatedMultiContent">\n\t\t\t\t{"template": [\n\t\t\t\t\tMultiContentEntryPixmapAlphaTest(pos = (12, 4), size = (32, 32), png = 0),\n\t\t\t\t\tMultiContentEntryText(pos = (58, 5), size = (440, 38), font=0, flags = RT_HALIGN_LEFT|RT_VALIGN_TOP, text = 1),\n\t\t\t\t\t],\n\t\t\t\t\t"fonts": [gFont("Regular", 22)],\n\t\t\t\t\t"itemHeight": 40\n\t\t\t\t}\n\t\t\t</convert>\n\t\t</widget>\t\n\t</screen>'


    def __init__(self, session, args = 0):
        self.session = session
        self.setup_title = _('Mediatomb Panel')
        Screen.__init__(self, session)
        l = []
        l.append(self.buildListEntry(_('Show Mediatomb status'), 'copyepg.png'))
        l.append(self.buildListEntry(_('Start Mediatomb'), 'fileoperations.png'))
        l.append(self.buildListEntry(_('Stop Mediatomb'), 'filter.png'))
    ...
    def openSelected(self):
        ...
        if menuIndex == 0:
			self.session.open(SFConsole,title = _("Show Mediatomb status"), cmdlist = ["/etc/init.d/mediatomb status"])
        elif menuIndex == 1:
